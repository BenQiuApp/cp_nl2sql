...
def check_part_acc(pred_queries, gt_queries, tables_list, valid_data):
	...
	for pred_qry, gt_qry, table_id, valid_d in zip(pred_queries, gt_queries, tables_list, valid_data):
		...
		strs_py3 = json.dumps(eval(str({'question': valid_d['question'], 'table': valid_d['table_id']})),
													ensure_ascii=False) + '\n' + \
	...
	print(
			'Sel-Num: %.3f, Sel-Col: %.3f, Sel-Agg: %.3f, W-Num: %.3f, W-Col: %.3f, W-Op: %.3f, W-Val: %.3f, W-Rel: %.3f, total_err: %.3f'
	...
	return np.array((sel_num_err, sel_err, agg_err, cond_num_err, cond_col_err, cond_op_err, cond_val_err,
										 cond_rela_err)), tot_err, tot_err / q_len  # 这里返回的都是总数哦，需要特殊处理成占比
